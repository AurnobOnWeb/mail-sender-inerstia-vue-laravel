...
const lightTheme = () => {
    isDark.value = false;
    DarkMode.value.setIsDark(isDark);
};
const DarkTheme = () => {
    isDark.value = true;
    DarkMode.value.setIsDark(isDark);
...
<template>
    <div
        v-if="Settings.isSettings"
        @click="Settings.isSettings = false"
        class="fixed inset-0 z-10 bg-primary-darker"
        style="opacity: 0.5"
        aria-hidden="true"
    ></div>
    <!-- Panel -->
    <Transition name="slide-fade-setting">
        <section
            tabindex="-1"
            v-if="Settings.isSettings"
            @keydown.escape="Settings.isSettings = false"
